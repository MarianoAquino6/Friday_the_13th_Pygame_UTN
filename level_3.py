...
def level_3(player_name, score):
    #---------------------------------------------READING AND CREATING LEVEL--------------------------------------------------------
    #READING LEVEL
    level = read_json(level="level_3")
    ...
    create_npc(taxi=level["enemies"]["taxi"], gang_members=level["enemies"]["gang_member"], npc_list=npc_list)
    create_items(beers= level["items"]["beer"], coins=level["items"]["coin"], traffic_lights=level["items"]["traffic_light"], item_object_list=item_object_list)
    create_platforms(lava_or_spikes=level["platforms"]["street"], 
    ...
    while True:
        ...
        if paused:
            ...
            for enemy in npc_list:
                if type(enemy) == Taxi:
                    enemy.update_character(ms=time, invisible_rects=invisible_rects, traffic_light=traffic_light)
            ...
            if jason_voorhees.life < 1 or remaining_time < 1:
                graphic_user_interface.show_death_screen(level=3, score=jason_voorhees.score, player_name=player_name)
